...
void build_result_matrix(const double *partC, double *C, MPI_Comm grid_comm, const int dims[DIMENSIONS], int coords[DIMENSIONS], int n1, int n3, int comm_size);
...
void free_all(double *A, double *B, double *C, double *partA, double *partB, double *partC, MPI_Comm *grid_comm, MPI_Comm *col_comm, MPI_Comm *row_comm, int rank);
...
int main(int argc, char **argv)
{
    ...
    int n1 = (int)strtol(argv[1], nullptr, 10);
    int n2 = (int)strtol(argv[2], nullptr, 10);
    int n3 = (int)strtol(argv[3], nullptr, 10);
    ...
    auto *partA = new double[n1 * n2 / dims[1]];
    auto *partB = new double[n2 * n3 / dims[0]];
    auto *partC = new double[n1 * n3 / (dims[X] * dims[Y])];
